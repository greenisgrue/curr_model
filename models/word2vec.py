        ...
        if versions != '[]':
            ...
            if str(primary.get('uid')) != chosen_uid:
                ...
                if self.audience == 'Grundskola 7-9':
                    self.relevant_CI = self.relevant_CI.append(CI[(CI['value'] == 'Skillnader i språkanvändning beroende på syfte, mottagare och sammanhang. Språkets betydelse för att utöva inflytande.') | (CI['value'] == 'Ord och begrepp för att uttrycka känslor, kunskaper och åsikter. Ords och begrepps nyanser och värdeladdning. Bildspråk och idiomatiska uttryck.')], ignore_index=True)
                    self.relevant_CI_inc_titles = self.relevant_CI_inc_titles.append(CI[(CI['value'] == 'Skillnader i språkanvändning beroende på syfte, mottagare och sammanhang. Språkets betydelse för att utöva inflytande.') | (CI['value'] == 'Ord och begrepp för att uttrycka känslor, kunskaper och åsikter. Ords och begrepps nyanser och värdeladdning. Bildspråk och idiomatiska uttryck.')], ignore_index=True)
                    if 'Modersmål -  utom nationella minoritetsspråk' in self.subject:
                        self.relevant_CI = self.relevant_CI.append(CI[(CI['value'] == 'Uttal, betoning och satsmelodi i jämförelse med svenskan samt olika talade variationer av modersmålet.')], ignore_index=True)
                        self.relevant_CI_inc_titles = self.relevant_CI_inc_titles.append(CI[(CI['value'] == 'Uttal, betoning och satsmelodi i jämförelse med svenskan samt olika talade variationer av modersmålet.')], ignore_index=True)
